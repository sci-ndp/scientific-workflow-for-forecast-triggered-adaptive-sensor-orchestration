...
def eval_condition(condition, df):
    ...
    if operator == 'IN':
        value_list = _parse_in_condition_values(value)
        if value_list is None:
            return pd.Series([False] * len(df), index=df.index)

        if any(isinstance(item, str) for item in value_list):
...
def _parse_in_condition_values(value):
    """
    Parse the right-hand side of an IN expression into a list without using eval.
    Accepts Python literals, JSON-style lists, or comma-separated tokens (with or without quotes).
    """
    if value is None:
        return []
